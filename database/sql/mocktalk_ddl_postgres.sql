-- PostgreSQL DDL for mocktalkback (entity-aligned, v3 design)
...
CREATE TABLE tb_file_classes (
    ...
    updated_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
    deleted_at      TIMESTAMPTZ,
    CONSTRAINT uq_tb_file_classes_code UNIQUE (file_class_code)
);
...
CREATE TABLE tb_files (
    ...
    updated_at   TIMESTAMPTZ NOT NULL DEFAULT now(),
    deleted_at   TIMESTAMPTZ,
    CONSTRAINT fk_tb_files_tb_file_classes
        FOREIGN KEY (file_class_id) REFERENCES tb_file_classes (file_class_id)
);

CREATE INDEX ix_tb_files_class_id ON tb_files (file_class_id);
...
CREATE TABLE tb_user_files (
    ...
    created_at    TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at    TIMESTAMPTZ NOT NULL DEFAULT now(),
    CONSTRAINT fk_tb_user_files_tb_users
        FOREIGN KEY (user_id) REFERENCES tb_users (user_id),
    CONSTRAINT fk_tb_user_files_tb_files
...
COMMENT ON COLUMN tb_file_classes.file_class_code IS '파일 분류 코드(유니크, 시스템 키)';
...
COMMENT ON CONSTRAINT uq_tb_file_classes_code ON tb_file_classes IS '파일 분류 코드 유니크 제약';
...
COMMENT ON COLUMN tb_files.storage_key   IS '실제 저장소 키/경로(S3 key 등, 유니크 정책은 서비스에서 관리)';
...
COMMENT ON INDEX ix_tb_files_class_id IS '분류별 파일 조회 최적화(file_class_id)';

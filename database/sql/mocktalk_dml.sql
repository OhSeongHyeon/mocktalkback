-- Seed data for mocktalk (minimum 10 rows per table, except tb_role/tb_users)
...
SELECT gs, gs FROM generate_series(1, 10) gs;
...
SELECT ((gs - 1) % 3) + 1, gs FROM generate_series(1, 10) gs;
...
SELECT gs, ((gs - 1) % 3) + 1 FROM generate_series(1, 10) gs;
